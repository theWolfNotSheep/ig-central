...
- [ ] Existing services begin emitting via `gls-platform-audit` library. Phase 1.12 PR5: `gls-governance-enforcement` is the first service migrated — added a thin `EnforcementAuditEmitter` helper that constructs envelopes from the small set of fields each emit site has on hand, then dual-writes alongside every existing `auditEventRepository.save(...)` call. Eight emit sites covered (PII_SENSITIVITY_ESCALATED ×2, GOVERNANCE_APPLIED, STORAGE_TIER_MIGRATED, STORAGE_MIGRATION_FAILED, DOCUMENT_DISPOSED, DOCUMENT_ARCHIVED, DOCUMENT_ANONYMISED, DISPOSITION_FAILED). The `AuditEmitter` bean is optional via `ObjectProvider`; absent → no-op (legacy save still fires, no audit data lost). Future PRs migrate `gls-app-assembly` document/classification flows, then connectors.
