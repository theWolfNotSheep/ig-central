...
`igc-app-assembly`) and publishes the resulting ONNX artefact to
MinIO under a versioned key.

The companion `igc-bert-inference` JVM service then loads the ONNX
...
4. **Publish** (`publish.py`): upload the ONNX file + label-mapping
   ...
   `${IGC_BERT_ARTIFACTS_BUCKET}/${trainerVersion}/${modelVersion}/model.onnx`.
...
IGC_BERT_ARTIFACTS_BUCKET=igc-bert-artifacts \
IGC_BERT_TRAINER_TOP_N=3 \
IGC_BERT_TRAINER_MIN_SAMPLES_PER_CLASS=50 \
igc-bert-trainer
```
...
- `train.py` — HuggingFace fine-tune + Optimum ONNX export. Real but
  ...
  than `IGC_BERT_TRAINER_MIN_SAMPLES_PER_CLASS * top_n`.
