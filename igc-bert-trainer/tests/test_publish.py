...
from igc_bert_trainer.publish import ArtefactPublisher, build_metadata
...
def test_publish_uploads_onnx_and_metadata_under_versioned_prefix(tmp_path: Path):
    ...
    publisher = ArtefactPublisher(
        client=client, bucket="igc-bert-artifacts", trainer_version="0.1.0"
    )
    ...
    assert result.bucket == "igc-bert-artifacts"
    ...
    assert onnx_call.args[0] == "igc-bert-artifacts"
