...
- `openapi.yaml` — initial OpenAPI 3.1.1 declaration of `gls-extraction-ocr`. Three operations:
...
- Initial supported mime types: `image/png`, `image/jpeg`, `image/tiff`, `image/bmp`, `application/pdf` (image-only). Text-extractable PDFs route to `gls-extraction-tika` instead.
