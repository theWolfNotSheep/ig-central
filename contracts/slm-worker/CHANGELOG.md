...
- `openapi.yaml` — initial OpenAPI 3.1.1 declaration of `igc-slm-worker`. Six operations:
  ...
  - `GET /v1/jobs/{nodeRunId}` — poll an async classification job. Same shape as `igc-extraction-audio` and `igc-classifier-router`'s async surface (CSV #47).
...
- Phase 1.5 first cut: schema lands now; the `igc-slm-worker` module ships with a stub `NotConfiguredSlmService` that returns `SLM_NOT_CONFIGURED` 503 until either Anthropic Haiku creds or a local Ollama endpoint is wired.
