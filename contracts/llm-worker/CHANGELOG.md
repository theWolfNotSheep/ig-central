...
- `openapi.yaml` — initial OpenAPI 3.1.1 declaration of `igc-llm-worker`. Same shape as `igc-slm-worker` (the SLM contract was authored first as the template) so the cascade router can dispatch to either tier through identical client code.
...
- Phase 1.6 first cut: contract + JVM module + a stub `NotConfiguredLlmService` returning `LLM_NOT_CONFIGURED` 503. The Anthropic / Ollama integration lifts from `igc-llm-orchestration` in PR2.
