...
Run as ``gls-bert-trainer`` (from the installed package) or
``python -m gls_bert_trainer`` (from a checkout).
...
logger = logging.getLogger("gls_bert_trainer")
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="gls-bert-trainer-") as tmp:
