...
from gls_bert_trainer import config as cfg
...
def test_load_with_minimal_env(monkeypatch):
    ...
    assert config.artifacts_bucket == "gls-bert-artifacts"
...
def test_load_overrides_via_env(monkeypatch):
    ...
    monkeypatch.setenv("GLS_BERT_TRAINER_TOP_N", "5")
    monkeypatch.setenv("GLS_BERT_TRAINER_MIN_SAMPLES_PER_CLASS", "100")
    monkeypatch.setenv("GLS_BERT_TRAINER_BASE_MODEL", "custom/model")
    monkeypatch.setenv("GLS_BERT_TRAINER_EPOCHS", "5")
