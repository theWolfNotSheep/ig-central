 ...
 * {@code igc.router.async.*} when load profiling lands.
 */
@Configuration
public class AsyncConfig {
    ...
    public Executor routerAsyncExecutor(
            @Value("${igc.router.async.core-size:4}") int coreSize,
            @Value("${igc.router.async.max-size:8}") int maxSize,
            @Value("${igc.router.async.queue-capacity:32}") int queueCapacity) {
