 ...
 * ({@code igc.router.policy.refresh-seconds}, default 60s). On a
 ...
 * {@code igc-slm-worker}: reads the raw {@code pipeline_blocks}
 * collection via {@link MongoTemplate} so the router doesn't depend
 * on {@code igc-governance}'s class shape.
 */
@Component
public class RouterPolicyResolver {
    ...
    public RouterPolicyResolver(
            MongoTemplate mongo,
            @Value("${igc.router.policy.block-name:default-router}") String blockName,
            @Value("${igc.router.policy.refresh-seconds:60}") int refreshSeconds) {
        this.mongo = mongo;
