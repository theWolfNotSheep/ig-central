...
public class RouterHttpConfig {
    ...
    @Bean
    @ConditionalOnProperty(prefix = "igc.router.cascade.bert", name = "enabled", havingValue = "true")
    public BertHttpDispatcher bertHttpDispatcher(
            @Value("${igc.router.cascade.bert.url:http://igc-bert-inference:8080}") String url,
            @Value("${igc.router.cascade.bert.timeout-ms:30000}") int timeoutMs,
            ObjectMapper mapper) {
    ...
    @Bean
    @ConditionalOnProperty(prefix = "igc.router.cascade.slm", name = "enabled", havingValue = "true")
    public SlmHttpDispatcher slmHttpDispatcher(
            @Value("${igc.router.cascade.slm.url:http://igc-slm-worker:8080}") String url,
            @Value("${igc.router.cascade.slm.timeout-ms:60000}") int timeoutMs,
            ObjectMapper mapper) {
        HttpClient client = HttpClient.newBuilder()
                .connectTimeout(Duration.ofSeconds(10))
                .build();
    ...
    @Bean
    @ConditionalOnProperty(prefix = "igc.router.cascade.llm-http", name = "enabled", havingValue = "true")
    ...
    public LlmHttpDispatcher llmHttpDispatcher(
            @Value("${igc.router.cascade.llm-http.url:http://igc-llm-worker:8080}") String url,
            @Value("${igc.router.cascade.llm-http.timeout-ms:90000}") int timeoutMs,
            @Value("${igc.router.cascade.llm-http.budget-fallback-retry-after:PT1H}") Duration budgetFallback,
