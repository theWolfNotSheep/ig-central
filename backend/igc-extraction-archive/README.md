---
title: igc-extraction-archive
lifecycle: forward
---

# igc-extraction-archive

Phase 1.1 service. Synchronous archive unpack — `.zip`, `.mbox`, `.pst`. Streams bytes from MinIO, walks the archive **one level deep**, lands each direct child as a fresh MinIO object, and returns the list of child `documentRef`s. The caller (`igc-app-assembly` / orchestrator) commits child `DocumentModel` rows and publishes the per-child `igc.documents.ingested` events. Recursion through nested archives happens via the normal pipeline routing — a `.zip` containing `.mbox` re-routes the `.mbox` back through this service on its own request, with its own `nodeRunId`. Decision: CSV #43.

Cloned from the `igc-extraction-tika` reference pattern (Phase 0.5) and `docs/service-template.md`.
...
- `backend/igc-extraction-tika/` — reference implementation; clone source for the impl PRs.
