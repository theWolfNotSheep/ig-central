...
public class TikaExtractionService {
     ...
     * {@code TextExtractionService} default in {@code igc-document-processing}
     ...
     * {@code igc.extraction.tika.max-characters} when present, else
     * {@link #DEFAULT_MAX_CHARACTERS}. */
    public TikaExtractionService(@Value("${igc.extraction.tika.max-characters:500000}") int maxCharacters) {
        if (maxCharacters <= 0) {
