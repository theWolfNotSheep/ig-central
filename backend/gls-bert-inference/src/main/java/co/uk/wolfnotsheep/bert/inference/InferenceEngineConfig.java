 ...
 * {@code gls.bert.inference.engine=none} → {@link NotLoadedInferenceEngine}.
...
public class InferenceEngineConfig {
    ...
    public InferenceEngine inferenceEngine(
            @Value("${gls.bert.inference.engine:none}") String engine) {
        if (!"none".equalsIgnoreCase(engine)) {
            ...
            log.warn("bert: gls.bert.inference.engine={} is requested but no implementation is wired in this build — falling back to not-loaded",
                    engine);
        }
