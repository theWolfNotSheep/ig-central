 ...
 * {@code gls.extraction.audio.provider}. Default {@code none} → the
...
public class AudioBackendConfig {
    ...
    public AudioTranscriptionService audioTranscriptionService(
            @Value("${gls.extraction.audio.provider:none}") String provider,
            @Value("${gls.extraction.audio.openai.api-key:}") String apiKey,
            @Value("${gls.extraction.audio.openai.endpoint:https://api.openai.com/v1/audio/transcriptions}") String endpoint,
            @Value("${gls.extraction.audio.openai.model:whisper-1}") String model,
            @Value("${gls.extraction.audio.openai.timeout:PT10M}") Duration timeout) {
