 ...
 * tune via {@code gls.extraction.audio.async.*} when load profiling
 * lands.
 */
@Configuration
public class AsyncConfig {
    ...
    public Executor audioAsyncExecutor(
            @Value("${gls.extraction.audio.async.core-size:4}") int coreSize,
            @Value("${gls.extraction.audio.async.max-size:8}") int maxSize,
            @Value("${gls.extraction.audio.async.queue-capacity:32}") int queueCapacity) {
