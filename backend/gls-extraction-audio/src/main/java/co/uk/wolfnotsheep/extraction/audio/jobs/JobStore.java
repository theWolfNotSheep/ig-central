 ...
 * completed async run gets the cached result; a {@code Prefer: respond-async}
 * after a completed sync run gets the COMPLETED job via the poll URL.
 */
@Service
public class JobStore {
    ...
    public JobStore(
            JobRepository repo,
            @Value("${gls.extraction.audio.jobs.ttl:PT24H}") Duration ttl) {
        this.repo = repo;
        this.ttl = ttl;
    }

    /**
     * Try to claim {@code nodeRunId} for a fresh transcription.
     * ACQUIRED → caller proceeds; RUNNING → caller short-circuits
     * (sync 409 / async returns the existing pending job); COMPLETED
     * → caller returns cached result; FAILED → caller treats as
     * RUNNING-equivalent for sync (re-raises a RUNNING-like error)
     * but for async returns the failed job for visibility.
     */
    public JobAcquisition tryAcquire(String nodeRunId) {
        Optional<JobRecord> existing = repo.findById(nodeRunId);
        if (existing.isPresent()) {
            JobRecord row = existing.get();
        ...
        try {
            ...
            return repo.findById(nodeRunId)
                    .map(r -> switch (r.status()) {
                        case PENDING, RUNNING -> JobAcquisition.running(r);
                        case COMPLETED -> JobAcquisition.completed(r);
                        case FAILED -> JobAcquisition.failed(r);
                    })
