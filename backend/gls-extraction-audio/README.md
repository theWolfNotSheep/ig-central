---
title: gls-extraction-audio
lifecycle: forward
---

# gls-extraction-audio
...
|---|---|---|
| `gls.extraction.audio.provider` | `none` | `openai` swaps in `OpenAiWhisperService`. |
| `gls.extraction.audio.openai.api-key` | _empty_ | Provide via `OPENAI_API_KEY`. Falls back to not-configured if blank. |
| `gls.extraction.audio.openai.model` | `whisper-1` | OpenAI Whisper model id. |
| `gls.extraction.audio.openai.endpoint` | `https://api.openai.com/v1/audio/transcriptions` | Overridable for proxies / Azure-OpenAI. |
| `gls.extraction.audio.openai.timeout` | `PT10M` | HTTP timeout per request. |
| `gls.extraction.audio.async.max-size` | `8` | Upper bound on concurrent async jobs per replica. |
| `gls.extraction.audio.max-source-bytes` | `524288000` (500 MB) | 413 cap. |
| `gls.extraction.audio.inline-byte-ceiling` | `262144` (256 KB) | Inline vs textRef threshold per CSV #19. |
