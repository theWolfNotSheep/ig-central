 ...
 * {@code gls.enforcement.worker.async.*} once load profiling lands.
 */
@Configuration
public class AsyncConfig {
    ...
    public Executor enforcementAsyncExecutor(
            @Value("${gls.enforcement.worker.async.core-size:4}") int coreSize,
            @Value("${gls.enforcement.worker.async.max-size:8}") int maxSize,
            @Value("${gls.enforcement.worker.async.queue-capacity:32}") int queueCapacity) {
