 ...
 * is chosen by {@code igc.audit.collector.tier1-backend}.
 *
...
public class Tier1Consumer {
     ...
     * 5 s; tune via {@code igc.audit.collector.tier1.poll-interval}.
     * The {@code lockAtMostFor} caps how long a stuck replica can
     * hold leadership; default 1 minute.
     */
    @Scheduled(
            fixedDelayString = "${igc.audit.collector.tier1.poll-interval:PT5S}",
            initialDelayString = "${igc.audit.collector.tier1.initial-delay:PT5S}")
    @SchedulerLock(name = "audit-tier1-leader",
            lockAtMostFor = "${igc.audit.collector.tier1.lock-at-most-for:PT1M}",
            lockAtLeastFor = "${igc.audit.collector.tier1.lock-at-least-for:PT0S}")
