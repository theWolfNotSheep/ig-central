 ...
 * {@code igc.audit.collector.tier1-backend=mongo} (or absent). Future
 * PRs may add an S3-Object-Lock-backed implementation behind the
 * same interface — selectable via
 * {@code igc.audit.collector.tier1-backend=s3-object-lock}.
 */
@Service
@ConditionalOnProperty(name = "igc.audit.collector.tier1-backend", havingValue = "mongo", matchIfMissing = true)
