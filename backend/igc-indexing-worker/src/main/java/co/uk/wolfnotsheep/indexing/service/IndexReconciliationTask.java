 ...
 * by setting {@code igc.indexing.reconciliation.auto-fix=true}.
 *
 * <p>Detection:
 * <ul>
...
public class IndexReconciliationTask {
    ...
    public IndexReconciliationTask(
            ...
            @Value("${igc.indexing.reconciliation.drift-threshold:10}") long driftThreshold,
            @Value("${igc.indexing.reconciliation.auto-fix:false}") boolean autoFix,
     ...
     * {@code igc.indexing.reconciliation.interval} (e.g. {@code PT1H} for
     * hourly). Initial-delay 5 min so app startup isn't immediately followed
     * by a heavy survey.
     */
    @Scheduled(
            fixedDelayString = "${igc.indexing.reconciliation.interval:PT24H}",
            initialDelayString = "${igc.indexing.reconciliation.initial-delay:PT5M}")
    @SchedulerLock(name = "index-reconciliation",
            lockAtMostFor = "${igc.indexing.reconciliation.lock-at-most-for:PT30M}",
            lockAtLeastFor = "${igc.indexing.reconciliation.lock-at-least-for:PT0S}")
