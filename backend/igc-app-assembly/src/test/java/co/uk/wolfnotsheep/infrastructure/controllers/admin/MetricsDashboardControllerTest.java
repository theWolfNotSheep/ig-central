...
class MetricsDashboardControllerTest {
    ...
    void dlqReplaySection_groupsByQueueModeOutcome() {
        registry.counter("dlq.replay",
                "queue", "igc.documents.dlq", "mode", "real", "outcome", "replayed").increment(4);
        registry.counter("dlq.replay",
                "queue", "igc.documents.dlq", "mode", "dry_run", "outcome", "replayed").increment(2);
        registry.counter("dlq.replay",
                "queue", "igc.pipeline.dlq", "mode", "real", "outcome", "skipped").increment(1);
        ...
        MetricsDashboardController.DlqReplayActivity docs = rows.stream()
                .filter(r -> r.queue().equals("igc.documents.dlq")).findFirst().orElseThrow();
        ...
        MetricsDashboardController.DlqReplayActivity pipe = rows.stream()
                .filter(r -> r.queue().equals("igc.pipeline.dlq")).findFirst().orElseThrow();
