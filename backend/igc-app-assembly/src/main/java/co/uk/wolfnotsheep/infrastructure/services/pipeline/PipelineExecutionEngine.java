...
public class PipelineExecutionEngine {
    ...
    // Phase 1.3 cutover: optional sync HTTP path through igc-classifier-router.
    ...
    // Phase 1.10 PR3 — optional sync HTTP path through igc-enforcement-worker.
    // Bean is conditionally registered via @ConditionalOnProperty; default OFF
                    ...
                    case "SYNC_LLM" -> {
                        ...
                        if (routerClient != null) {
                            statusNotifier.emitLog(docId,
                                    doc.getOriginalFileName() != null ? doc.getOriginalFileName() : "",
                                    "LLM_CALL", "INFO",
                                    "Classify dispatched via igc-classifier-router for node '" + node.label() + "'",
                                    null);
