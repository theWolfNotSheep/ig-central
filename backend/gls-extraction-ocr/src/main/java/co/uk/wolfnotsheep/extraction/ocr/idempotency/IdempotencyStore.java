...
/**
 * Idempotency store for the OCR service per CSV #16. Mirrors Tika's
 * pattern; collection is `ocr_idempotency` with a Mongo TTL on
 * `expiresAt`.
 */
@Service
public class IdempotencyStore {
    ...
    public IdempotencyStore(
            ...
            @Value("${gls.extraction.ocr.idempotency.ttl:PT24H}") Duration ttl) {
        this.repo = repo;
        this.ttl = ttl;
    }
