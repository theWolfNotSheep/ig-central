 ...
 * downstream consumer ({@code igc-audit-collector}) already deduplicates
 * on via {@code eventId}.
 *
 * <p>Scheduling uses Spring's {@code @Scheduled}. Consumers must declare
 * {@code @EnableScheduling} on their application — {@code igc-app-assembly}
...
public class OutboxRelay {
    ...
    @Scheduled(fixedDelayString = "${igc.platform.audit.relay.poll-interval:PT5S}")
    @SchedulerLock(name = "igc-audit-outbox-relay",
            lockAtMostFor = "${igc.platform.audit.relay.lock-at-most-for:PT5M}",
            lockAtLeastFor = "${igc.platform.audit.relay.lock-at-least-for:PT0S}")
