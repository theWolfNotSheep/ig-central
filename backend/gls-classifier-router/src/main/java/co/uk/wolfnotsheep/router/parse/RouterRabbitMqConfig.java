 ...
 * {@code gls.router.cascade.llm.enabled=true}; without that flag the
 * mock cascade stays in place.
 *
 * <p>The exchange + queue declarations are deliberately compatible
 * with the existing {@code gls-llm-orchestration} topology — the
 * exchange is shared (passive declare via Spring's idempotent
 * topology), the dispatch queue ({@code gls.pipeline.llm.jobs}) is
 ...
 */
@Configuration
@ConditionalOnProperty(prefix = "gls.router.cascade.llm", name = "enabled", havingValue = "true")
public class RouterRabbitMqConfig {
     ...
     * Matches the {@code gls.config.changed} pattern used elsewhere
     * in the v2 stack.
     */
    @Bean
    ...
    public LlmDispatchCascadeService llmDispatchCascadeService(
            ...
            @Value("${gls.router.cascade.llm.timeout:PT60S}") Duration timeout) {
     ...
     * worker uses ({@code gls-llm-orchestration}'s
