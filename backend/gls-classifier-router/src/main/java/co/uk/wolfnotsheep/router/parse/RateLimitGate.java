 ...
 *
 * <p>Set {@code gls.router.rate-limit.permits=0} (default) to disable
 * enforcement entirely — the gate becomes a pass-through.
 *
 * <p>Same shape as the existing {@code RateLimitGate} in
 * {@code gls-llm-worker} (Phase 1.6 PR4). Per-replica, not
...
public class RateLimitGate {
    ...
    public RateLimitGate(
            @Value("${gls.router.rate-limit.permits:0}") int permits,
            @Value("${gls.router.rate-limit.wait-ms:0}") long waitMs,
        ...
        if (this.permits > 0) {
            ...
            log.info("router: rate limit disabled (gls.router.rate-limit.permits=0)");
        }
    }
