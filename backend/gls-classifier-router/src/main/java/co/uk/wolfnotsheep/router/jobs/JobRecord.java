...
/**
 * Mongo-mapped row in {@code router_jobs}. One row per
 * {@code nodeRunId}; the row backs both the idempotency cache (sync
 * path) and the async-job poll surface — sync and async share state
 * per CSV #47.
 *
 * <p>{@code resultJson} is the serialised {@code ClassifyResponse} for
 * COMPLETED rows. {@code errorCode} / {@code errorMessage} populated
 * for FAILED rows. PENDING / RUNNING rows have neither.
 */
@Document(collection = "router_jobs")
