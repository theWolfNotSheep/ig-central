...
class MetaControllerTest {
    ...
    void capabilities_advertises_service_identity_and_default_tier() {
        ...
        MetaController controller = new MetaController("igc-extraction-audio", "0.0.1-SNAPSHOT", backend);
        ...
        assertThat(body.getService()).isEqualTo("igc-extraction-audio");
    ...
    void health_returns_UP_when_backend_is_ready() {
        AudioTranscriptionService backend = mock(AudioTranscriptionService.class);
        when(backend.isReady()).thenReturn(true);
        MetaController controller = new MetaController("igc-extraction-audio", "0.0.1", backend);
