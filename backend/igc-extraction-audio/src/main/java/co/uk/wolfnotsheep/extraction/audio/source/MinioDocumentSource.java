...
public class MinioDocumentSource implements DocumentSource {
    ...
    @Override
    @Observed(name = "minio.fetch", contextualName = "minio-fetch",
            lowCardinalityKeyValues = {"component", "minio"})
    public InputStream open(DocumentRef ref) {
        if (ref.etag() != null && !ref.etag().isBlank()) verifyEtag(ref);
        try {
            return minio.getObject(GetObjectArgs.builder()
                    .bucket(ref.bucket()).object(ref.objectKey()).build());
        } catch (ErrorResponseException e) {
            if (NOT_FOUND_CODE.equals(e.errorResponse().code())) throw new DocumentNotFoundException(ref);
    ...
    public long sizeOf(DocumentRef ref) {
        try {
            ...
            if (NOT_FOUND_CODE.equals(e.errorResponse().code())) return -1L;
    ...
    private void verifyEtag(DocumentRef ref) {
        try {
            ...
            if (!ref.etag().equals(actual)) throw new DocumentEtagMismatchException(ref, actual);
        } catch (ErrorResponseException e) {
            if (NOT_FOUND_CODE.equals(e.errorResponse().code())) throw new DocumentNotFoundException(ref);
