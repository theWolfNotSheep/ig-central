...
/**
 * Mongo-mapped row in {@code audio_jobs}. One row per
 * {@code nodeRunId}; the row backs both the idempotency cache (sync
 * path) and the async-job poll surface.
 *
 * <p>{@code resultJson} is the serialised {@code ExtractResponse} for
 * COMPLETED rows. {@code errorCode} / {@code errorMessage} populated
 * for FAILED rows. PENDING / RUNNING rows have neither.
 */
@Document(collection = "audio_jobs")
