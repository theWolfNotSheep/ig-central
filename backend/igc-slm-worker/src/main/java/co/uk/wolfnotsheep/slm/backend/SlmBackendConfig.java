 ...
 *
 * <ol>
 *     <li>{@code igc.slm.worker.backend=anthropic} +
 ...
 *     <li>{@code igc.slm.worker.backend=ollama} +
...
public class SlmBackendConfig {
    ...
    public SlmService slmService(
            @Value("${igc.slm.worker.backend:none}") String backend,
            ...
            @Value("${igc.slm.worker.anthropic.model:claude-haiku-4-5}") String anthropicModel,
            @Value("${igc.slm.worker.anthropic.temperature:0.1}") double anthropicTemperature,
            @Value("${igc.slm.worker.anthropic.max-tokens:1024}") int anthropicMaxTokens,
            @Value("${igc.slm.worker.ollama.model:llama3.1:8b}") String ollamaModel,
            @Value("${igc.slm.worker.ollama.temperature:0.1}") double ollamaTemperature,
            @Value("${igc.slm.worker.ollama.num-ctx:32768}") int ollamaNumCtx,
        ...
        if ("anthropic".equalsIgnoreCase(backend)) {
            ...
            if (model == null) {
                log.warn("slm: igc.slm.worker.backend=anthropic but AnthropicChatModel bean is missing — set ANTHROPIC_API_KEY. Falling back to not-configured.");
        ...
        if ("ollama".equalsIgnoreCase(backend)) {
            ...
            if (model == null) {
                log.warn("slm: igc.slm.worker.backend=ollama but OllamaChatModel bean is missing — set spring.ai.ollama.base-url to a reachable Ollama instance. Falling back to not-configured.");
                return new NotConfiguredSlmService();
            }
            if (resolver == null) {
                log.warn("slm: PromptBlockResolver bean is missing — Mongo not wired? Falling back to not-configured.");
