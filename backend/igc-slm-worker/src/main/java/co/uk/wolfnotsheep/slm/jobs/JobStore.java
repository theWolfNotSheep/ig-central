 ...
 * via {@code nodeRunId} per CSV #47. Same shape as
 * {@code igc-classifier-router} and {@code igc-extraction-audio} —
 * lifted deliberately so observers and operational tooling can treat
 * the v2 services identically.
 */
@Service
public class JobStore {
    ...
    public JobStore(
            JobRepository repo,
            @Value("${igc.slm.worker.jobs.ttl:PT24H}") Duration ttl) {
        this.repo = repo;
        this.ttl = ttl;
    }
