 ...
 * the full ES walk. Tune via {@code gls.indexing.worker.async.*}
...
public class AsyncConfig {
    ...
    public Executor indexingAsyncExecutor(
            @Value("${gls.indexing.worker.async.core-size:2}") int coreSize,
            @Value("${gls.indexing.worker.async.max-size:4}") int maxSize,
            @Value("${gls.indexing.worker.async.queue-capacity:8}") int queueCapacity) {
