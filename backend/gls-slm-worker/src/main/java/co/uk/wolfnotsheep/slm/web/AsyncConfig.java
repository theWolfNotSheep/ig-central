 ...
 * thread for backend-call duration; tune via {@code gls.slm.worker.async.*}
...
public class AsyncConfig {
    ...
    public Executor slmAsyncExecutor(
            @Value("${gls.slm.worker.async.core-size:4}") int coreSize,
            @Value("${gls.slm.worker.async.max-size:8}") int maxSize,
            @Value("${gls.slm.worker.async.queue-capacity:32}") int queueCapacity) {
