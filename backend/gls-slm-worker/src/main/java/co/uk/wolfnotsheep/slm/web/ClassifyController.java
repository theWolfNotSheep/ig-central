...
import co.uk.wolfnotsheep.slm.api.ClassifyApi;
import co.uk.wolfnotsheep.slm.audit.SlmEvents;
...
import co.uk.wolfnotsheep.slm.backend.SlmResult;
import co.uk.wolfnotsheep.slm.backend.SlmService;
import co.uk.wolfnotsheep.slm.jobs.JobAcquisition;
...
import co.uk.wolfnotsheep.slm.model.JobAccepted;
import co.uk.wolfnotsheep.slm.model.PromptBlockRef;
...
public class ClassifyController implements ClassifyApi {
    ...
    public ClassifyController(
            ...
            @Value("${spring.application.name:gls-slm-worker}") String serviceName,
            @Value("${gls.slm.worker.build.version:0.0.1-SNAPSHOT}") String serviceVersion,
    ...
    private ResponseEntity<ClassifyResponse> handleSyncAcquired(
        ...
        try {
            ...
            metrics.recordSuccess(timer, body == null || body.getBackend() == null
                    ? null : body.getBackend().name());
    ...
    private ClassifyResponse doClassify(String traceparent, ClassifyRequest request) {
        ...
        SlmResult result = backend.classify(blockId, blockVersion, text);
    ...
    private static ClassifyResponse.BackendEnum toApiBackend(SlmResult result) {
        return switch (result.backend()) {
            case ANTHROPIC_HAIKU -> ClassifyResponse.BackendEnum.ANTHROPIC_HAIKU;
            case OLLAMA -> ClassifyResponse.BackendEnum.OLLAMA;
            case NONE -> null;
        };
    }
    ...
    private void cacheCompleted(String nodeRunId, ClassifyResponse response) {
        ...
        try {
            ...
            log.warn("slm cache write failed for nodeRunId={}: {}", nodeRunId, e.getMessage());
        }
    }

    private void emitCompleted(ClassifyRequest request, String traceparent,
                               SlmResult result, long byteCount, long durationMs) {
        AuditEmitter emitter = auditEmitterProvider.getIfAvailable();
        if (emitter == null) return;
        PromptBlockRef block = request.getBlock();
        try {
            emitter.emit(SlmEvents.completed(
            ...
            log.warn("audit emit (SLM_COMPLETED) failed for nodeRunId={}: {}",
    ...
    private void emitFailed(ClassifyRequest request, String traceparent,
        ...
        try {
            emitter.emit(SlmEvents.failed(
                    serviceName, serviceVersion, instanceId,
                    nodeRunId, traceparent, errorCode, cause.getMessage()));
        } catch (RuntimeException e) {
            log.warn("audit emit (SLM_FAILED) failed for nodeRunId={}: {}",
    ...
    private static String errorCodeFor(Throwable cause) {
        if (cause instanceof SlmNotConfiguredException) return "SLM_NOT_CONFIGURED";
        if (cause instanceof BlockUnknownException) return "SLM_BLOCK_UNKNOWN";
        if (cause instanceof JobInFlightException) return "IDEMPOTENCY_IN_FLIGHT";
        if (cause instanceof java.io.UncheckedIOException) return "SLM_DEPENDENCY_UNAVAILABLE";
        return "SLM_UNEXPECTED";
    }
}
