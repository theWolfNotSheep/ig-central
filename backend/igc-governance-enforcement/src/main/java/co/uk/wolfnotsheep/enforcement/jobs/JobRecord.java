...
/**
 * Mongo-mapped row in {@code enforcement_jobs}. One row per
 * {@code nodeRunId}; backs both the idempotency cache (sync path) and
 * the async-job poll surface — sync and async share state per CSV #47.
 * Same shape as the sibling slm-worker / classifier-router stores.
 */
@Document(collection = "enforcement_jobs")
