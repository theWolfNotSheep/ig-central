 ...
 * probe endpoint) at {@code igc.llm.worker.mcp.probe.interval} (default
...
public class McpAvailabilityProbe {
    ...
    public McpAvailabilityProbe(
            ...
            @Value("${igc.llm.worker.mcp.probe.path:/actuator/health}") String probePath,
            @Value("${igc.llm.worker.mcp.probe.timeout:PT3S}") Duration timeout) {
    ...
    @Scheduled(
            fixedDelayString = "${igc.llm.worker.mcp.probe.interval:PT30S}",
            initialDelayString = "${igc.llm.worker.mcp.probe.initial-delay:PT30S}")
    public void probe() {
