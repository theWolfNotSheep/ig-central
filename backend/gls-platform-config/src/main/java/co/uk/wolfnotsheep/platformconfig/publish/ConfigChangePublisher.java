...
public class ConfigChangePublisher {
     ...
     * {@code gls.platform.config.publisher.retry.flush-interval} interval
     * (default 30 s). Stops on first AMQP failure so a stuck broker
     * doesn't burn through the whole buffer in one tick.
     */
    @Scheduled(fixedDelayString = "${gls.platform.config.publisher.retry.flush-interval:PT30S}",
            initialDelayString = "${gls.platform.config.publisher.retry.initial-delay:PT30S}")
