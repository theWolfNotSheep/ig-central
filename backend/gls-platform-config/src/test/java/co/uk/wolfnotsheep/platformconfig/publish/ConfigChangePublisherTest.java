...
class ConfigChangePublisherTest {
    ...
    void successful_publish_does_not_buffer() {
        doNothing().when(rabbitTemplate).send(eq("gls.config"), eq("config.changed"), any(Message.class));

        publisher.publishSingle("TAXONOMY", "tax-1", ChangeType.UPDATED);

        verify(rabbitTemplate, times(1)).send(eq("gls.config"), eq("config.changed"), any(Message.class));
    ...
    void AMQP_failure_buffers_event_for_retry() {
        doThrow(new AmqpException("broker down"))
                .when(rabbitTemplate).send(eq("gls.config"), eq("config.changed"), any(Message.class));
    ...
    void scheduled_flush_drains_buffer_when_broker_recovers() {
        ...
        verify(rabbitTemplate, times(4)).send(eq("gls.config"), eq("config.changed"), any(Message.class));
    }
    ...
    void flush_stops_at_first_failure_so_stuck_broker_doesnt_burn_buffer() {
        ...
        verify(rabbitTemplate, times(3)).send(eq("gls.config"), eq("config.changed"), any(Message.class));
        assertThat(publisher.retryBufferSize()).isEqualTo(2);
    ...
    void direct_publish_with_traceparent_is_buffered_intact() {
        ...
        verify(rabbitTemplate, times(2)).send(eq("gls.config"), eq("config.changed"), any(Message.class));
    }
}
