 ...
 *
 * <p>Set {@code gls.llm.worker.rate-limit.permits=0} (default) to
...
public class RateLimitGate {
    ...
    public RateLimitGate(
            @Value("${gls.llm.worker.rate-limit.permits:0}") int permits,
            @Value("${gls.llm.worker.rate-limit.wait-ms:0}") long waitMs) {
        ...
        if (this.permits > 0) {
            ...
            log.info("llm: rate limit disabled (gls.llm.worker.rate-limit.permits=0)");
        }
    }
