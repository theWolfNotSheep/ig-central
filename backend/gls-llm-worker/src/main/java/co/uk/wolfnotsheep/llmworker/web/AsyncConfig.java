 ...
 * thread for backend-call duration; tune via {@code gls.llm.worker.async.*}
...
public class AsyncConfig {
    ...
    public Executor llmAsyncExecutor(
            @Value("${gls.llm.worker.async.core-size:4}") int coreSize,
            @Value("${gls.llm.worker.async.max-size:8}") int maxSize,
            @Value("${gls.llm.worker.async.queue-capacity:32}") int queueCapacity) {
