 ...
 *
 * <p>Set {@code gls.llm.worker.budget.daily-token-cap=0} (default)
...
public class CostBudgetTracker {
    ...
    public CostBudgetTracker(
            @Value("${gls.llm.worker.budget.daily-token-cap:0}") long dailyTokenCap) {
    ...
    CostBudgetTracker(long dailyTokenCap, Clock clock) {
        ...
        if (dailyTokenCap > 0) {
            ...
            log.info("llm: cost budget disabled (gls.llm.worker.budget.daily-token-cap=0)");
        }
    }
